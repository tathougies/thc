...
module Text.CYK.FingerTree (
    ...
    -- * Zipper
    FingerTreeZ(..), openZipper, zipperToList
    -- * Example
    -- $example
    ) where
...
data FingerTreeZ v a
  = FingerTreeZ v a (FingerTreeZ v a)
  | FingerTreeDone
  deriving Show

openZipper :: (Monoid v, Measured v a) => (v -> Bool) -> FingerTree v a -> FingerTreeZ v a
openZipper _ Empty = FingerTreeDone
openZipper p xs
  | p (measure xs) = runCont (reset (openZipperTree yield p mempty xs >> pure FingerTreeDone)) id
  | otherwise = FingerTreeDone

openZipperTree :: (Monoid v, Measured v a)
               => (v -> a -> Cont (FingerTreeZ v b) a) -> (v -> Bool) -> v -> FingerTree v a -> Cont (FingerTreeZ v b) ()
openZipperTree _ _ _ Empty = error "openZipperTree: Empty"
openZipperTree yield' p v (Single x)
  | not (p v) && p (v <> measure x) = yield' v x >> pure ()
  | otherwise = pure ()
openZipperTree yield' p i (Deep _ pr m sf)
  | p vpr = openDigit yield' p i pr >> traverseWPTree (traverseWPNode yield') vpr m >> traverseWPDigit yield' vm sf >> pure ()
  | p vm  = openZipperTree (openNode yield' p) p vpr m >> traverseWPDigit yield' vm sf >> pure ()
  | otherwise = openDigit yield' p vm sf
  where vpr = i `mappend` measure pr
        vm = vpr `mappend` measure m

openNode :: (Monoid v, Measured v a)
         => (v -> a -> Cont (FingerTreeZ v b) a) -> (v -> Bool) -> v -> Node v a -> Cont (FingerTreeZ v b) (Node v a)
openNode yield' p i n@(Node2 _ a b)
  | p ia = yield' i a >> yield' ia b >> pure n
  | p ib = yield' ia b >> pure n
  | otherwise = pure n
  where ia = i `mappend` measure a
        ib = ia `mappend` measure b
openNode yield' p i n@(Node3 _ a b c)
  | p ia = yield' i a >> yield' ia b >> yield' ib c >> pure n
  | p ib = yield' ia b >> yield' ib c >> pure n
  | p ic = yield' ib c >> pure n
  | otherwise = pure n
  where ia = i `mappend` measure a
        ib = ia `mappend` measure b
        ic = ib `mappend` measure c

openDigit :: (Monoid v, Measured v a)
          => (v -> a -> Cont (FingerTreeZ v b) a) -> (v -> Bool) -> v -> Digit a -> Cont (FingerTreeZ v b) ()
openDigit yield' p i (One a) = yield' i a >> pure ()
openDigit yield' p i (Two a b)
  | p ia = yield' i a >> yield' ia b >> pure ()
  | p ib = yield' ia b >> pure ()
  | otherwise = pure ()
  where ia = i `mappend` measure a
        ib = ia `mappend` measure b
openDigit yield' p i (Three a b c)
  | p ia = yield' i a >> yield' ia b >> yield' ib c >> pure ()
  | p ib = yield' ia b >> yield' ib c >> pure ()
  | p ic = yield' ib c >> pure ()
  | otherwise = pure ()
  where ia = i `mappend` measure a
        ib = ia `mappend` measure b
        ic = ib `mappend` measure c
openDigit yield' p i (Four a b c d)
  | p ia = yield' i a >> yield' ia b >> yield' ib c >> yield' ic d >> pure ()
  | p ib = yield' ia b >> yield' ib c >> yield' ic d >> pure ()
  | p ic = yield' ib c >> yield' ic d >> pure ()
  | p id = yield' ic d >> pure ()
  | otherwise = pure ()
  where ia = i `mappend` measure a
        ib = ia `mappend` measure b
        ic = ib `mappend` measure c
        id = ic `mappend` measure d

yield :: v -> b -> Cont (FingerTreeZ v b) b
yield v a = shift $ \k -> pure (FingerTreeZ v a (k a))

zipperToList :: FingerTreeZ v b -> [ b ]
zipperToList FingerTreeDone = []
zipperToList (FingerTreeZ _ x n) = x:zipperToList n

-- data FingerTreeDir v a
--   = GoRight
--   | Search (v -> Bool)
--   | Replace a
--   deriving Show
...
-- data FingerTreeZ v a
--   = FingerTreeDone !(FingerTree v a)
--   | FingerTreeZ !v a (FingerTreeZ v a)

-- openZipper :: FingerTree v a -> FingerTreeZ v a
-- openZipper ft = reset (FingerTreeDone <$> zipThrough ft)

-- zipThrough _ Empty = pure (Replce Empty)
-- zipThrough _ (Single a) = do
--   next <- yield (measure a) a
--   case next of
--     GoRight -> pure (Replace (Single a))
--     Search pred
--       | pred (measure v) -> zipThrough (Replace (Single a))
--       | otherwise -> pure (Replace (Single a))
--     Replace b -> zipThrough (Single b)
-- zipThrough zipBranchLeft d@(Deep total l spine r) =
--   zipBranchLeft l >>= handleLeftDir
--   where
--     handleLeftDir GoRight = zipThisSpine
--     handleLeftDir (Replace b) = zipThrough zipBranchLeft (Deep (measure b <> measure spine <> measure r) b spine r)
--     handleLeftDir (Search pred) = doSearch pred

--     zipThisSpine = do
--       next <- zipSpine zipBranchLeft spine
--       case next of
--         GoRight -> zipThisRight
--         Search pred -> doSearch pred
--         Replace a -> 

data Test a = Test Int a deriving Show

instance Measured (Sum Int) (Test a) where
  measure (Test x _) = Sum x
