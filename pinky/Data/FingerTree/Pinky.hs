...
foldMapTree RuntimeZ {} f (Tree a) = f a
foldMapTree l@(RuntimeSucc {}) f (Tree (Node2 _ a b)) = foldMapTree l' f a `mappend` foldMapTree l' f b
  where l' = prevDepth l
foldMapTree l@(RuntimeSucc {}) f (Tree (Node3 _ a b c)) = foldMapTree l' f a `mappend` foldMapTree l' f b `mappend` foldMapTree l' f c
  where l' = prevDepth l
...
foldrTree RuntimeZ{} f x (Tree a) = {-# SCC foldrTreeOne #-} f a x
foldrTree l@(RuntimeSucc {}) f x (Tree (Node2 _ a b)) = {-# SCC foldrTreeNode2 #-} next (next x b) a
  where !l' = prevDepth l
        next = foldrTree l' f
foldrTree l@(RuntimeSucc {}) f x (Tree (Node3 _ a b c)) = {-# SCC foldrTreeNode3 #-} next (next (next x c) b) a
  where !l' = prevDepth l
        next = foldrTree l' f
...
measureTree RuntimeZ {} (Tree x) = measure x
measureTree (RuntimeSucc {}) (Tree n) = measure n
...
zeroDepth = let z = RuntimeZ (next z)
            in z
  where
...
traverseTree RuntimeZ {} f (Tree x) = Tree <$> f x
traverseTree l@(RuntimeSucc {}) f (Tree (Node2 _ a b)) = treeDigit <$> (node2 l' <$> traverseTree l' f a <*> traverseTree l' f b)
  where l' = prevDepth l
traverseTree l@(RuntimeSucc {}) f (Tree (Node3 _ a b c)) = treeDigit <$> (node3 l' <$> traverseTree l' f a <*> traverseTree l' f b <*> traverseTree l' f c)
  where l' = prevDepth l
...
traverseWPTree RuntimeZ {} f v (Tree x) = Tree <$> f v x
traverseWPTree l@(RuntimeSucc {}) f v (Tree (Node2 _ a b)) =
  treeDigit <$> (node2 l' <$> traverseWPTree l' f v a <*> traverseWPTree l' f va b)
  where
    l' = prevDepth l
    va = v `mappend` measureTree l' a
traverseWPTree l@(RuntimeSucc {}) f v (Tree (Node3 _ a b c)) =
...
toListTree RuntimeZ {} (Tree t) next = t:next
toListTree l@(RuntimeSucc {}) (Tree (Node2 _ a b)) next =
  toListTree l' a (toListTree l' b next)
  where !l' = prevDepth l
toListTree l@(RuntimeSucc {}) (Tree (Node3 _ a b c)) next =
